...
CSV_OUT  = r"/home/clab/Downloads/jiaxin_temporal/Droso/data/full_wing_features.csv"
...
def extract_intervein_features(wing_mask, skel, max_regions=6, vein_radius=3):
    ...
    参数：
      wing_mask  : 二值翅膀掩膜 (bool / 0-1)
      skel       : 一像素宽的 skeleton (bool)
      max_regions: 保留的 intervein 区域数（按面积排序）
      vein_radius: 膨胀 skeleton 的半径，用于近似翅脉宽度

    返回：
      一个 dict，比如：
        iv1_area_frac, iv1_ecc, iv1_sol,
        iv2_area_frac, ...
    """
    ...
    # 先构造默认值（全 NaN），防止有的翅膀 intervein 数量不够
    feats = {}
    ...
    # 4. 取前 max_regions 个 intervein 区域，提特征
...
def main():
    ...
    for fname in file_list:
        ...
        if mask is None:
            ...
            # 几何特征（可以改用这个 skel，而不是里面再 skeletonize 一次）
            geom = extract_geom_features(mask)  # 如果愿意，可以改成传 skel 进去
            ...
            # intervein 特征
            intervein_feats = extract_intervein_features(mask, skel,
                                                         max_regions=6,
                                                         vein_radius=3)
        ...
        rec = meta
        rec.update(geom)
        rec.update({f"efd_{i}": float(efd_vec[i]) for i in range(len(efd_vec))})
        rec.update(skel_feats)
        rec.update(intervein_feats)  # ⭐ 新增这一行
